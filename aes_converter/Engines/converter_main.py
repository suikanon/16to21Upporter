...
def dependency_check():
    ...
    try:
        ...
        missing_dependencies = [packageName for (importName, packageName) in dependencies if
                                not importlib.util.find_spec(importName)]
...
def run_type_request(argv):
    if len(argv) > 1 and argv[1] in ["2", "1", "0"]:
        return argv[1]
...
def convert_players():
    ...
    for player_folder in os.listdir("players_to_convert"):
        ...
        # modelFile = model2fmdl.loadFmdl(filename)
...
def convert_teams21():
    from lib.convertTeam21 import convertTeam

    # Check if the "EDIT00000000" file exists in the "exports_to_convert" folder
    input_savefile_path = ijoin("exports_to_convert21", "EDIT00000000")
    if input_savefile_path is None:
        print("- An \"EDIT00000000\" savefile with the team's data is needed")
        print("- in the \"exports_to_convert\" folder.")
        print("- Please copy it from the PES19 savedata folder.")
    ...
    OUTPUT_FOLDER = "exports_converted21"
    # Check if the output folder exists and create it if it doesn't
    if not os.path.isdir(OUTPUT_FOLDER):
        os.mkdir(OUTPUT_FOLDER)

    # For each export folder in the "exports_to_convert" folder
    for export_folder in os.listdir("exports_to_convert21"):
        export_folder_path = os.path.join("exports_to_convert21", export_folder)
        if not os.path.isdir(export_folder_path):
            continue
